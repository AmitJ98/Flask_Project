...
from flask_blog import app, db, bycrypt, mail
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Your accout had been created!!','success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bycrypt.check_password_hash(user.password,form.password.data):
            ...
            flash('Login Unssuccessful. please check email and password', 'danger')
...
def save_picture(form_picture):
    ...
    _, f_ext = os.path.splitext(form_picture.filename) #_varible that is not needed
...
def user_posts(username):
    ...
    posts = Post.query.filter_by(author=user)\
        ...
        .paginate(page = page ,per_page=5) # paginate able to sparate for pages
...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Requset', sender='amitj.you98@gmail.com', recipients=[user.email])
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your Password had been updated!!','success')
        return redirect(url_for('login'))
